export const ABOUT_PREMIUM_3 = [
  {
    id: "about_premium_3",
    type: "about_us_theme_premium_3",

    data: {
      /* ================= HERO ================= */
      heroTitle: "Our Story Built for Buy & Selling a Vehicle",
      heroDescription: `<p>Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed. Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed. Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed.</p>`,
      heroImageTemplateId1: { id: 1, imageUrl: "/video.mp4" },
      heroImageTemplateId2: {
        id: 2,
        imageUrl:
          "https://images.unsplash.com/photo-1503376780353-7e6692767b70?w=900&q=90",
      },
      /* ================= MISSION ================= */
      missionTitle: "Our Mission",
      missionDesc: `<p>Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed. Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed.</p>`,
      missionTemplate1: {
        id: 3,
        imageUrl:
          "https://images.unsplash.com/photo-1503376780353-7e6692767b70?w=900&q=90",
      },
      /* ================= VISION ================= */
      visionTitle: "Our Vision",
      visionDesc: `<p>Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed. Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec.</p>`,
      visionTemplate1: {
        id: 4,
        imageUrl:
          "https://images.unsplash.com/photo-1555215695-3004980ad54e?w=900&q=90",
      },
      /* ================= STATS ================= */
      aboutUsDescription: `<p>Lorem ipsum dolor sit amet consectetur. Odio at dolor ut donec. Sapien platea nec urna ut est sed. Lorem ipsum dolor sit amet consectetur.</p>`,
      stats: [
        { number: "150K+", label: "Active Users Worldwide" },
        { number: "$2B+", label: "Transactions Processed" },
        { number: "98%", label: "Customer Satisfaction" },
        { number: "100+", label: "Team Members" },
      ],
      /* ================= SERVICES ================= */
      servicesTitle: "What We Do",
      servicesDesc: `<p>Enterprise-grade digital products designed to scale globally with security, speed and reliability.</p>`,
      services: [
        {
          icon: `<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-80q-139-35-229.5-159.5T160-516v-244l320-120 320 120v244q0 152-90.5 276.5T480-80Zm0-84q104-33 172-132t68-220v-189l-240-90-240 90v189q0 121 68 220t172 132Zm0-316Zm-76 112 198-198-57-56-141 142-70-71-57 56 127 127Z"/></svg>`,
          title: "Secure Payments",
          desc: "PCI-DSS compliant global payment systems.",
        },
        {
          icon: `<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M320-240 80-480l240-240 57 57-184 184 183 183-56 56Zm320 0-57-57 184-184-183-183 56-56 240 240-240 240Z"/></svg>`,
          title: "Global Infrastructure",
          desc: "99.99% uptime cloud deployment in 12 regions.",
        },
        {
          icon: `<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm-40-360v-240h80v207l154 154-57 57-177-178Z"/></svg>`,
          title: "Growth Tools",
          desc: "Smart CRM, analytics and automation funnels.",
        },
        {
          icon: `<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M784-120 532-372q-30 24-69 38t-83 14q-109 0-184.5-75.5T120-580q0-109 75.5-184.5T380-840q109 0 184.5 75.5T640-580q0 44-14 83t-38 69l252 252-56 56ZM380-400q75 0 127.5-52.5T560-580q0-75-52.5-127.5T380-760q-75 0-127.5 52.5T200-580q0 75 52.5 127.5T380-400Z"/></svg>`,
          title: "AI Optimization",
          desc: "AI powered performance & conversion engines.",
        },
      ],
    },
    rules: {
      heroTitle: { min: 10, max: 40 },
      heroDescription: { min: 50, max: 800 },
      aboutUsDescription: { min: 50, max: 800 },
      missionTitle: { min: 10, max: 30 },
      missionDesc: { min: 50, max: 400 },
      visionTitle: { min: 10, max: 30 },
      visionDesc: { min: 50, max: 400 },
      servicesTitle: { min: 10, max: 30 },
      servicesDesc: { min: 50, max: 600 },
      heroImageTemplateId1: { min: 5, max: 2000 },
      heroImageTemplateId2: { min: 5, max: 2000 },
      missionTemplate1: { min: 5, max: 2000 },
      visionTemplate1: { min: 5, max: 2000 },
      customHeroImage1: { min: 5, max: 2000 },
      customMissionImage1: { min: 5, max: 2000 },
      customVisionImage1: { min: 5, max: 2000 },
    },
    arrayRules: {
      stats: {
        number: { min: 1, max: 3 },
        label: { min: 5, max: 25 },
      },
      services: {
        title: { min: 5, max: 30 },
        desc: { min: 10, max: 60 },
      },
    },
  },
];
