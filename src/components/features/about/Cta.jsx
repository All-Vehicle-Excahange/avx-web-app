import React from "react";
import Link from "next/link";
import { CarFront, Store, ArrowRight } from "lucide-react";
import Button from "@/components/ui/button";

const renderIcon = (IconComponent, props) => {
  if (!IconComponent) return null;
  if (React.isValidElement(IconComponent)) {
    return React.cloneElement(IconComponent, props);
  }
  const Icon = IconComponent;
  return <Icon {...props} />;
};

export default function Cta({
  buyerLabel = "For Buyers",
  buyerTitle,
  buyerDescription = "Discover verified vehicles through structured, transparent listings.",
  buyerLink = "/search",
  buyerButtonText = "Enter Marketplace",
  buyerIcon = CarFront,
  consultantLabel = "For Consultants",
  consultantTitle,
  consultantDescription = "Build visibility and credibility on a structured, performance-led platform.",
  consultantLink = "/consultant",
  consultantButtonText = "Start Your Storefront",
  consultantIcon = Store,
}) {
  const finalBuyerTitleDesktop = buyerTitle || (
    <>
      Browse <br /> Vehicles
    </>
  );
  const finalBuyerTitleMobile = buyerTitle || "Browse Vehicles";

  const finalConsultantTitleDesktop = consultantTitle || (
    <>
      Become a <br /> Consultant
    </>
  );
  const finalConsultantTitleMobile = consultantTitle || "Become a Consultant";

  return (
    <>
      {/* Desktop Design (styled like BecameBanner, full-width) */}
      <section className="hidden md:block bg-fourth relative z-10 -mb-6 md:-mb-10">
        <div className="mx-auto w-full h-auto">
          <div className="flex flex-col md:flex-row overflow-hidden">
            
            {/* ================= BUYERS ================= */}
            <Link
              href={buyerLink}
              className="
                group
                flex-1
                flex flex-col
                transition-all duration-500 ease-out
                hover:bg-white/20
                hover:flex-[1.3]
                px-10 py-16 lg:py-20
                text-center
                border-b md:border-b-0 md:border-r border-primary/30
              "
            >
              <div className="flex-1">
                <div className="max-w-lg mx-auto">
                  <div className="flex items-center justify-start gap-10 mb-6">
                    <div className="h-20 w-20 rounded-full border mt-8 border-primary/30 flex items-center justify-center">
                      {renderIcon(buyerIcon, {
                        size: 32,
                        className: "text-primary",
                        strokeWidth: 1.7,
                      })}
                    </div>

                    <div className="text-left">
                      <p className="text-primary text-md tracking-[0.35em] uppercase font-semibold relative inline-block">
                        {buyerLabel}
                        <span className="absolute left-0 -bottom-2 h-[2px] w-1/2 bg-linear-to-r from-neutral-100 to-transparent rounded-full"></span>
                      </p>

                      <div className="text-2xl sm:text-3xl font-bold font-primary text-primary mt-4 text-left">
                        {finalBuyerTitleDesktop}
                      </div>
                    </div>
                  </div>
                </div>

                <p className="mt-4 max-w-lg text-base text-left text-primary/80 mx-auto leading-relaxed">
                  {buyerDescription}
                </p>
              </div>

              <div className="mt-10">
                <div className="max-w-lg mx-auto text-left">
                  <div className="group/btn inline-flex items-center bg-transparent hover:bg-white border border-white/40 hover:border-transparent text-white hover:text-secondary rounded-full p-1.5 pr-6 shadow-xl transition-all duration-300 group-hover:scale-105">
                    <div className="bg-white/10 group-hover/btn:bg-blue-50 p-2.5 rounded-full mr-4 transition-colors duration-300">
                      {renderIcon(buyerIcon, {
                        size: 24,
                        className: "text-white group-hover/btn:text-secondary",
                        strokeWidth: 2.5,
                      })}
                    </div>
                    <span className="font-bold text-lg whitespace-nowrap transition-colors duration-300">
                      {buyerButtonText}
                    </span>
                  </div>
                </div>
              </div>
            </Link>

            {/* ================= CONSULTANTS ================= */}
            <Link
              href={consultantLink}
              className="
                group
                flex-1
                flex flex-col
                transition-all duration-500 ease-out
                hover:bg-white/20
                hover:flex-[1.3]
                px-10 py-16 lg:py-20
                text-center
              "
            >
              <div className="flex-1">
                <div className="max-w-lg mx-auto">
                  <div className="flex items-center justify-start gap-10 mb-6">
                    <div className="h-20 w-20 mt-8 rounded-full border border-primary/30 flex items-center justify-center">
                      {renderIcon(consultantIcon, {
                        size: 32,
                        className: "text-primary",
                        strokeWidth: 1.7,
                      })}
                    </div>

                    <div className="text-left">
                      <p className="text-primary text-md tracking-[0.35em] uppercase font-semibold relative inline-block">
                        {consultantLabel}
                        <span className="absolute left-0 -bottom-2 h-[2px] w-1/2 bg-linear-to-r from-neutral-100 to-transparent rounded-full"></span>
                      </p>

                      <div className="text-2xl font-bold font-primary sm:text-3xl text-primary mt-4 text-left">
                        {finalConsultantTitleDesktop}
                      </div>
                    </div>
                  </div>
                </div>

                <p className="mt-4 text-base text-primary/80 max-w-lg mx-auto leading-relaxed text-left">
                  {consultantDescription}
                </p>
              </div>

              <div className="mt-10">
                <div className="max-w-lg mx-auto text-left">
                  <div className="group/btn2 inline-flex items-center bg-transparent hover:bg-white border border-white/40 hover:border-transparent text-white hover:text-secondary rounded-full p-1.5 pr-3 shadow-xl transition-all duration-300 group-hover:scale-105">
                    <div className="bg-white/10 group-hover/btn2:bg-blue-50 p-3 rounded-full mr-2 transition-colors duration-300">
                      {renderIcon(consultantIcon, {
                        size: 24,
                        className: "text-white group-hover/btn2:text-secondary",
                        strokeWidth: 2,
                      })}
                    </div>
                    <span className="font-bold text-lg mr-4 whitespace-nowrap">
                      {consultantButtonText}
                    </span>
                    <div className="mx-3 h-6 w-[1.5px] bg-white/20 group-hover/btn2:bg-gray-200 mr-5 transition-colors duration-300"></div>
                    <div className="w-10 h-10 rounded-full border border-white/40 group-hover/btn2:border-secondary/20 flex items-center justify-center transition-all duration-300 group-hover:scale-110">
                      <ArrowRight
                        size={20}
                        className="text-white group-hover/btn2:text-secondary"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </Link>

          </div>
        </div>
      </section>

      {/* Mobile Design (styled like ConsultBanner, full-width) */}
      <section className="block md:hidden bg-fourth">
        {/* ================= BUYERS MOBILE ================= */}
        <div className="w-full h-[280px] overflow-hidden relative bg-fourth border-b border-white/20">
          <div className="relative w-full h-full mx-auto flex items-center">
            <div className="relative z-10 w-full px-6 flex flex-col items-start gap-4">
              <div>
                <h3 className="text-2xl font-bold text-white mb-2 tracking-tight">
                  {finalBuyerTitleMobile}
                </h3>
                <p className="text-gray-200 text-sm leading-relaxed">
                  {buyerDescription}
                </p>
              </div>
              <Button
                href={buyerLink}
                variant="ghost"
                size="sm"
              >
                {buyerButtonText}
              </Button>
            </div>
          </div>
        </div>

        {/* ================= CONSULTANTS MOBILE ================= */}
        <div className="w-full h-[280px] overflow-hidden relative bg-fourth">
          <div className="relative w-full h-full mx-auto flex items-center">
            <div className="relative z-10 w-full px-6 flex flex-col items-start gap-4">
              <div>
                <h3 className="text-2xl font-bold text-white mb-2 tracking-tight">
                  {finalConsultantTitleMobile}
                </h3>
                <p className="text-gray-200 text-sm leading-relaxed">
                  {consultantDescription}
                </p>
              </div>
              <Button
                href={consultantLink}
                variant="ghost"
                size="sm"
              >
                {consultantButtonText}
              </Button>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
