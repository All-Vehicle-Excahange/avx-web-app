...
export default function AvxInspected() {
  ...
  React.useEffect(() => {
    ...
    if (!isLoading && (!Array.isArray(vehicles) || vehicles.length === 0) && activeType === "4-Wheeler") {
      checkedFourWheelerEmpty.current = true;
      setActiveType("2-Wheeler");
    }
  }, [vehicles, isLoading, activeType]);
        ...
        <div className="grid sm:items-center grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {isLoading ? (
            ...
            vehicles.map((vehicle) => (
              <VehicleCard key={vehicle.id} data={vehicle} source="home" />
            ))
          ) : (
